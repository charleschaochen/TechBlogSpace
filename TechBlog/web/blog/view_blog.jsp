...
<html>
	<head>
		<title>Charles's Tech Blog - <%=article.getArticleTitle()%></title>
	...
	<body>
			...
			<div class="main_content">
				...
				<div class="blogs_container">
					...
					<div class="blog_box">
						...
						<div class="blog_title">
							...
							<div class="blog_author_pic">
								<img src="<%=article.getAuthor().getImage()%>" />
							</div>
							<div class="blog_title_info">
								<span class="blog_title_text"><a><%=article.getArticleTitle()%></a>
								</span>
								<br />
								<span class="blog_post_info"> By <a><%=article.getAuthor().getName()%></a>&nbsp;&nbsp;|&nbsp;&nbsp;
									Post in <a
									href="/blog.shtml?category=<%=article.getCategory().getCategoryId()%>"><%=article.getCategory().getCategoryName()%></a>&nbsp;&nbsp;|&nbsp;&nbsp;
									Views(<span class="text_bold_green"><%=article.getViewTimes()%></span>)
								</span>
							</div>
							<a class="blog_reply_num" title="<%=article.getReplyTimes()%>条评论"
								href="#comments"> <%=article.getReplyTimes()%></a>
						</div>
						...
						<p class="blog_tags">
							...
							%>
							<a href="/blog.shtml?tag=<%=tag.getTagName()%>"><%=tag.getTagName()%></a>
							<%
								}
							%>
						</p>
					...
					<div class="blog_comment_list" id="comments">
						...
						: reply.getVisitorWebsite()%>"><%=reply.getVisitorName()%></a>
									</div>
		...
		<!-- Baidu Share script -->
		<script>window._bd_share_config={"common":{"bdSnsKey":{},"bdText":"我在Charles Tech Blog上看到一篇不错的博文，详情猛戳>>","bdMini":"2","bdMiniList":false,"bdPic":"","bdStyle":"0","bdSize":"32"},"share":{},"image":{"viewList":["tsina","weixin","sqq","qzone","tqq","tqf"],"viewText":"分享到：","viewSize":"16"},"selectShare":{"bdContainerClass":null,"bdSelectMiniList":["tsina","weixin","sqq","qzone","tqq","tqf"]}};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=86835285.js?cdnversion='+~(-new Date()/36e5)];</script>
